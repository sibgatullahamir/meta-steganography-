...
public class Steganograph
{
	...
	public static boolean embedMessage(File masterFile, File outputFile, String msg, int compression, String password)
	{
		...
		try
		{
			...
			// Convey the 32 bit message size into byte array
			tempByte= new byte[4];
			for(i=24, j=0; i>=0; i-=8, j++)
			{
				tempInt= messageSize;
	...
	// Inner class for description about Steganography in a frame
	...
	private static class AboutFrame extends JFrame implements ActionListener
	{
		...
		public AboutFrame()
		{
			setTitle("About Steganography "+ Steganograph.VERSION);
			...
			lblName= new MyJLabel("By: Sibgatullah Amir ", new Font("Monotype Corsiva", Font.PLAIN, 14), GREEN, BLACK);
			lblEmail= new MyJLabel("      adi31189@gmail.com", GREEN, BLACK);
			lblPhone= new MyJLabel("      +91 8791752495", GREEN, BLACK);
			if(new File("Images/About").exists())
				lblImage= new JLabel(new ImageIcon("Images/Myself"));
			else
			{
				lblImage= new MyJLabel("Steganography "+ Steganograph.VERSION, new Font("Times new roman", Font.PLAIN, 50), Color.red, Color.black);
			...
			panelAbout= UtilityOperations.createBorderedPanel(panelAbout, "About Steganography", 3, 3);
		...
		public void actionPerformed(ActionEvent e)
		{
			if(e.getSource()== btnVisit)
				new HTMLFrame("http://www.facebook.com/aditya.khanduja", true);
...
// Class to obtain information about a Steganography file
